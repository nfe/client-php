---
name: nfeio-php-sdk
description: "NFE.io PHP SDK (Composer nfe/nfe, namespace Nfe\\). MUST trigger when writing or reviewing PHP that integrates NFE.io: composer.json requires nfe/nfe or code references Nfe\\Client; or the user builds Brazilian fiscal-document automation in PHP (Laravel, Symfony, WHMCS, WooCommerce) — issuing or querying NFS-e/NF-e/NFC-e/CT-e/CFe-SAT, CNPJ/CPF/CEP lookups, tax calculation, webhook signature validation, certificate management, or invoice status polling. Applies even when the SDK isn't named. PHP idioms differ from the Node SDK (nfe-io)."
---
